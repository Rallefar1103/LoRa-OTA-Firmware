...
class diff_match_patch:
  ...
  def __init__(self):
    ...
    self._hexdig = '0123456789ABCDEFabcdef'
    self._hextochr = dict((a+b, chr(int(a+b,16)))
      ...
      while lineEnd < len(text) - 1:
        ...
        if line in lineHash:
          ...
          if len(lineArray) == maxLines:
            # Bail out at 65535 because unichr(65536) throws. - not in micropython
            line = text[lineStart:]
            lineEnd = len(text)
  ...
  def diff_cleanupSemanticLossless(self, diffs):
    ...
    def diff_cleanupSemanticScore(one, two):
      ...
      char1 = one[-1]
      char2 = two[0]
      nonAlphaNumeric1 = not char1.isalpha()
      nonAlphaNumeric2 = not char2.isalpha()
  ...
  def unquote(self, s):
      ...
      for item in res[1:]:
          try:
              s += self._hextochr[item[:2]] + item[2:]
          except KeyError:
              s += '%' + item
