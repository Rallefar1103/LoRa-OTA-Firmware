...
class TTN_APP:
  def __init__(self, app_id, dev_id, app_api_key):
    ...
    self.app_url = f"{root_url}/{app_id}"
    self.downlink_queue = []
    self.events = [f"{self.app_url}/uplink", f"{self.app_url}/join_accept",\
                    f"{self.app_url}/downlink_init", f"{self.app_url}/downlink_result",\
                          f"{self.app_url}/schedule_downlink_with_Hex?rx=<rxNum>&hex=<hexStr>"]
    
    if not os.path.exists(self.app_id):
